...
class dq_dob_age_check:
    def __init__(self, column, meta, add_info):
        ...
        self.logger = logging.getLogger("DQDOBAgeCheck")
        ...
        #self.logger.info(f"Initialized dq_dob_age_check class.")

    def test(self, df):
        try:

            self.logger.info(f"Applying DQ rule [dq_dob_age_check] on '{self.column_name}' column.")
            ...
            # Step 5: Check if customer_age is less than 16
            detailed_report_df.loc[:, "ERROR_DESCRIPTION"] = detailed_report_df[self.column_name].apply(
                lambda age: (
                    f"{self.column_name} is invalid: age is less than 16, got '{age}'"
                    if pd.notnull(age) and age < 16
                    else None
                )
            )
            ...
            self.logger.exception("An error occurred during the dq_dob_age_check function.")
            raise
