...
class dq_pan_number_check:
    ...
    def __init__(self, column, meta, add_info):
        ...
        self.logger = logging.getLogger("dq_pan_number_check")
    ...
    def test(self, df):
        try:
            self.logger.info(f"Applying DQ rule [dq_pan_number_check] on '{self.column_name}' column.")
            ...
            detailed_report_df.loc[:, "ERROR_DESCRIPTION"] = detailed_report_df[self.column_name].apply(
                lambda pan: (
                    f"{self.column_name} format is invalid: expected AAAAA9999A, got '{pan}'"
                    if pd.isnull(pan) or not bool(pan_pattern.match(str(pan).strip().upper()))
                    else None
                )
            )
            ...
            detailed_report_df.loc[:, "PRIMARY_KEY_VALUE"] = detailed_report_df.apply(
                ...
                if pd.isnull(row[self.column_name]) or not bool(pan_pattern.match(str(row[self.column_name]).strip().upper()))
                else None,
                axis=1
            )
            detailed_report_df.loc[:, "DQ_NAME"] = "dq_pan_number_check"
            ...
            self.logger.exception("An error occurred during the dq_pan_number_check function.")
            raise
